...
def _load_plans() -> list[PagePlan]:
    ...
    if not runtime_json.exists():
        raise RuntimeError(
            f"Missing runtime control file: {runtime_json}. Restore or recreate control/automation_runtime.json."
        )
...
def _generate_reel(plan: PagePlan) -> tuple[Path, str, str]:
    raise RuntimeError(
        "Legacy direct generation was removed. Use Airflow DAG slot tasks for generation, "
        "and keep this module only for shared Meta UI scheduling helpers."
    )
