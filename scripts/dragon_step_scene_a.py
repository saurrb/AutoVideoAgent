...
def main() -> None:
    ...
    prompt = (
        f"Generate one video. Duration {ctx['scene_a_duration_sec']} seconds. "
        f"Resolution {ctx['target_resolution']}. Aspect ratio {ctx['target_aspect_ratio']}. "
        f"Prompt: {ctx['scene_a_prompt']} Return only the generated video file path."
    )
    src = run_grok_and_wait_for_mp4(grok, prompt, cwd=root, home=grok_home, poll_sec=3.0, max_wait_sec=0)
    out_mp4 = run_dir / f"dragon_{ctx['row_id']}_scene_a.mp4"
