...
from lib.flows.page8_funny_universe import (  # noqa: E402
...
@dag(
    dag_id="page8_funny_universe_manual",
    description="Page 8 Funny Universe animated cartoon comic run with Grok content, Grok video, render, upload, and Telegram steps.",
    schedule=AIRFLOW_SCHEDULE,
    start_date=pendulum.datetime(2026, 6, 9, tz=LOCAL_TZ),
    catchup=False,
    max_active_runs=1,
    params={
        "target_dates": Param(
            ...
            type="array",
            title="Target dates",
            description="Dates in YYYY-MM-DD format, for example 2026-06-10. Leave empty to use tomorrow.",
        ),
        "slots": Param(
            [],
            type="array",
            title="Time slots",
            description="24-hour slots in HH:MM format, for example 09:00 or 21:00. Leave empty to use page defaults.",
        ),
    },
    tags=["page8", "funny_universe", "manual"],
)
def build_dag():
    ...
    def build_requests(dag_run=None):
        ...
        previous_done = ""
        skip_upload = bool(conf.get("skip_upload"))
        for item in requests:
            item["run_root"] = str(run_root)
            item["skip_upload"] = skip_upload
    ...
    generate_comic_content = task(task_id="generate_comic_content")(page8_generate_comic_content)
    grok_animated_video = task(task_id="grok_animated_video")(page8_grok_animated_video)
    render_video = task(task_id="render_video")(page8_render_video)
    upload_schedule = task(task_id="upload_schedule")(page8_upload_slot)
    telegram_notify = task(task_id="telegram_notify")(page8_telegram_slot)
    ...
    def slot_flow(request: dict):
        ...
        content = generate_comic_content(gated)
        animated = grok_animated_video(content)
        rendered = render_video(animated)
