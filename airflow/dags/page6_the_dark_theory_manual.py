...
from lib.flows.page6_the_dark_theory import (  # noqa: E402
...
PAGE_KEY = "page6_the_dark_theory"
...
@dag(
    dag_id="page6_the_dark_theory_manual",
    description="Page 6 The Dark Theory run with AI content, Speechma voice, Grok cinematic images, render, upload, and Telegram steps.",
    schedule=AIRFLOW_SCHEDULE,
    start_date=pendulum.datetime(2026, 6, 6, tz=LOCAL_TZ),
    catchup=False,
    max_active_runs=1,
    params={
        "target_dates": Param(
            ...
            type="array",
            title="Target dates",
            description="Dates in YYYY-MM-DD format, for example 2026-06-07. Leave empty to use tomorrow.",
        ),
        "slots": Param(
            [],
            type="array",
            title="Time slots",
            description="24-hour slots in HH:MM format, for example 09:30 or 21:30. Leave empty to use page defaults.",
        ),
    },
    tags=["page6", "the_dark_theory", "manual"],
)
def build_dag():
    ...
    generate_content = task(task_id="generate_content")(page6_generate_content)
    speechma_voice = task(task_id="speechma_voice")(page6_speechma_voice)
    grok_scene_images = task(task_id="grok_scene_images")(page6_grok_scene_images)
    render_video = task(task_id="render_video")(page6_render_video)
    upload_schedule = task(task_id="upload_schedule")(page6_upload_slot)
    telegram_notify = task(task_id="telegram_notify")(page6_telegram_slot)
