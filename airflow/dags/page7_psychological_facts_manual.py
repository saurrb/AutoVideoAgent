...
from lib.flows.page7_psychological_facts import (  # noqa: E402
...
PAGE_KEY = "page7_psychological_facts"
...
@dag(
    dag_id="page7_psychological_facts_manual",
    description="Page 7 Psychological Facts run with AI content, reference-style text render, upload, and Telegram steps.",
    schedule=AIRFLOW_SCHEDULE,
    start_date=pendulum.datetime(2026, 6, 8, tz=LOCAL_TZ),
    catchup=False,
    max_active_runs=1,
    params={
        "target_dates": Param(
            ...
            type="array",
            title="Target dates",
            description="Dates in YYYY-MM-DD format, for example 2026-06-09. Leave empty to use tomorrow.",
        ),
        "slots": Param(
            [],
            type="array",
            title="Time slots",
            description="24-hour slots in HH:MM format, for example 08:00 or 22:00. Leave empty to use page defaults.",
        ),
    },
    tags=["page7", "psychological_facts", "manual"],
)
def build_dag():
    ...
    generate_fact_content = task(task_id="generate_fact_content")(page7_generate_fact_content)
    render_video = task(task_id="render_video")(page7_render_video)
    upload_schedule = task(task_id="upload_schedule")(page7_upload_slot)
    telegram_notify = task(task_id="telegram_notify")(page7_telegram_slot)
    ...
    def slot_flow(request: dict):
        ...
        content = generate_fact_content(gated)
        rendered = render_video(content)
