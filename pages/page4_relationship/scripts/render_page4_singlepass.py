...
def _prepare_external_cmd(cmd: list[str]) -> list[str]:
    if os.name == "nt" or not cmd:
        return cmd
    prepared = list(cmd)
    exe = prepared[0]
    if exe.lower().endswith(".exe"):
        prepared[0] = _windows_to_wsl_path(exe)
        prepared[1:] = [_wsl_to_windows_path(x) for x in prepared[1:]]
    return prepared


def _ffmpeg_file_path(path: Path | str) -> str:
    return _wsl_to_windows_path(str(path))


def _ffmpeg_filter_path(path: Path | str) -> str:
    s = _ffmpeg_file_path(path).replace("\\", "/")
    return s.replace(":", r"\:")


def _local_path(path: Path | str) -> Path:
    if os.name != "nt":
        return Path(_windows_to_wsl_path(str(path)))
    return Path(path)


def _run(cmd: list[str]) -> None:
    actual = _prepare_external_cmd(cmd)
    p = subprocess.run(actual, capture_output=True, text=True)
    if p.returncode != 0:
...
def _build_oldproject_wordtimed_ass(
    ...
    old_root = _local_path(r"C:\Users\Saurabh\Downloads\videoAgent")
    ...
    py = "python.exe" if os.name != "nt" else sys.executable
...
def _wait_for_grok_event(grok_output_dir: Path, expected_count: int, max_wait_seconds: int) -> list[Path]:
    ...
    media = sorted(
        [p for p in grok_output_dir.glob("*") if p.suffix.lower() in {".mp4", ".mov", ".mkv", ".webm", ".png", ".jpg", ".jpeg", ".webp"}],
        key=lambda x: x.stat().st_mtime,
    )
    if len(media) >= expected_count:
        return media[:expected_count]
    raise RuntimeError(
        f"Grok outputs not ready. expected={expected_count}, found={len(media)}, dir={grok_output_dir}"
    )
